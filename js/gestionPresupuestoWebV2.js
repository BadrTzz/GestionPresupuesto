...
class MiGasto extends HTMLElement {
  ...
  _render() {
    ...
    editarBtn.onclick = () => {
      ...
      this._shadow.getElementById("fechaEdit").value = new Date(gasto.fecha)
        .toISOString()
        .split("T")[0];
      this._shadow.getElementById("etiqEdit").value =
    ...
    formEditar.onsubmit = (e) => {
      ...
      const todos = listarGastos().map((x) =>
        x.id === gasto.id ? gasto : x
      );
      sobrescribirGastos(todos);
      guardarGastosLocal();
...
document.addEventListener("gastoActualizado", () => {
  renderizarGasto();
  guardarGastosLocal();
});
